...
import 'history';
import { c as createStore, h as history, d as deliveryApi, p as pickProject, r as rootSaga } from './App-190603f7.js';
...
const webApp = (app, ReactApp, config) => {
  ...
  app.get('/*', (request, response) => {
    ...
    const store = createStore(withReducers, // fromJSLeaveImmer({}),
    fromJS({}), history({
      initialEntries: [url]
    })); // dispatch any global and non-saga related actions before calling our JSX

    const versionStatusFromHostname = deliveryApi.getVersionStatusFromHostname(request.hostname); // eslint-disable-next-line no-console
